...
<table width="100%" class="divHeader">
	...
	<table width="100%" class="table" style="font-size:12px">
			...
			<?php $dr_amt=0; $cr_amt=0; foreach($ref_bal[$journal_voucher_row->received_from_id] as $refbal): ?>
			<tr>
			...
			<?php 
				if($journal_voucher_row->cr_dr == 'Dr' ){ 
					$on_acc=$journal_voucher_row->amount-($dr_amt-$cr_amt);
					...
					<?php }} elseif( $journal_voucher_row->cr_dr == 'Cr'){
						$on_acc1=$journal_voucher_row->amount-($cr_amt-$dr_amt);
